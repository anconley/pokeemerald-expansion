...
static const u32 sPokecenterMonitor_Gfx[] = INCBIN_U32("graphics/field_effects/pics/pokecenter_monitor/0.4bpp",
...
{
    s32 spriteId = CreateMonPicSprite(species, 0, 0x8000, FALSE, x, y, 0, species);
...
{
    u16 spriteId = CreateMonPicSprite(species, otId, personality, FALSE, x, y, 0, species);
    PreservePaletteInWeather(IndexOfSpritePaletteTag(species) + 0x10);
    if (spriteId == 0xFFFF)
        return MAX_SPRITES;
    else
...
u8 FldEff_UseSurf(void)
{
    ...
    Overworld_ChangeMusicTo(gRegionalSurfMusic[GetCurrentRegion()]);
    return FALSE;
}
