...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyPicSprite(gTasks[taskId].tLotadSpriteId);
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    lotadSpriteId = CreateMonPicSprite(SPECIES_LOTAD, SHINY_ODDS, 0, TRUE, 100, 0x4B, 14, TAG_NONE);
    gSprites[lotadSpriteId].callback = SpriteCB_Null;
    gSprites[lotadSpriteId].oam.priority = 0;
    gSprites[lotadSpriteId].invisible = TRUE;
    gTasks[taskId].tLotadSpriteId = lotadSpriteId;
    brendanSpriteId = CreateTrainerSprite(gFacilityClassToPicIndex[FACILITY_CLASS_BRENDAN], 120, 60, 0, &gDecompressionBuffer[0]);
    gSprites[brendanSpriteId].callback = SpriteCB_Null;
    gSprites[brendanSpriteId].invisible = TRUE;
    gSprites[brendanSpriteId].oam.priority = 0;
    gTasks[taskId].tBrendanSpriteId = brendanSpriteId;
    maySpriteId = CreateTrainerSprite(gFacilityClassToPicIndex[FACILITY_CLASS_MAY], 120, 60, 0, &gDecompressionBuffer[TRAINER_PIC_SIZE]);
