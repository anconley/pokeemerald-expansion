...
static void Task_Hof_DisplayMon(u8 taskId)
{
    ...
    spriteId = CreateMonPicSprite(currMon->species, currMon->tid, currMon->personality, TRUE, startX, startY, currMonId, TAG_NONE);
...
static void Task_Hof_DisplayPlayer(u8 taskId)
{
    ...
    ShowBg(1);
    ShowBg(3);
    gTasks[taskId].tPlayerSpriteID = CreateTrainerPicSprite(PlayerGenderToFrontTrainerPicId(gSaveBlock2Ptr->playerGender), 120, 72, 6);
...
static void Task_Hof_HandleExit(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        FreeAndDestroyPicSprite(gTasks[taskId].tPlayerSpriteID);
        HideBg(0);
        HideBg(1);
        HideBg(3);
        FreeAllWindowBuffers();
...
static void Task_HofPC_DrawSpritesPrintText(u8 taskId)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++, currMon++)
    {
        if (currMon->species != 0)
        {
            ...
            spriteId = CreateMonPicSprite(currMon->species, currMon->tid, currMon->personality, FALSE, posX, posY, i, TAG_NONE);
